...
/***************************************************************************
    ...
    Controls execution of the core sbc8080 system.
...
// ======================> sbc8080_machine_manager

class sbc8080_machine_manager : public machine_manager
{
private:
	DISABLE_COPYING(sbc8080_machine_manager);
	// construction/destruction
	sbc8080_machine_manager(emu_options &options, osd_interface &osd) : machine_manager(options, osd) { }
public:
	static sbc8080_machine_manager *instance(emu_options &options, osd_interface &osd)
	{
		if (!m_manager)
		{
			m_manager = new sbc8080_machine_manager(options, osd);
		}
		return m_manager;
	}

	static sbc8080_machine_manager *instance() { return m_manager; }

	~sbc8080_machine_manager() { delete m_manager;  m_manager = nullptr; }

	int execute()
	{
		auto system = &GAME_NAME(sbc8080);
...
private:
	static sbc8080_machine_manager* m_manager;
...
sbc8080_machine_manager* sbc8080_machine_manager::m_manager = nullptr;
...
int emulator_info::start_frontend(emu_options &options, osd_interface &osd, std::vector<std::string> &args)
{
	...
	sbc8080_machine_manager::instance(options,osd)->start_http_server();
	sbc8080_machine_manager::instance(options,osd)->execute();
	return 0;
}
...
const char * emulator_info::get_appname_lower() { return "sbc8080"; }
